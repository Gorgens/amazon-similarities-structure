...
(Implement) Retirar espécies com zero de densidade
(Implement) Rodar randomForest para dados10, e dados70
(Implement) Rodar randomForest para esp 10, e esp70

Remove provinces with lower few inventored sites.
...
```{r}
rfAgrupamento = randomForest(factor(Province_1)~., data = dfAgrupamento, importance=TRUE, scale = TRUE)
```

```{r}
kable(importance(rfAgrupamento), digits = 2, col.names = c('Madeira', 'Pará', 'Rondônia', 'Roraima', 'Xingu-Tapajos', 'Decrease Accuracy', 'Decrease Gini') ,caption = 'Variable importace to classfy by province, and for general random forest performance.')
```
